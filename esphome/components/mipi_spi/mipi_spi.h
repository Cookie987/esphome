...
class MipiSpiBuffer
 ...
 public:
  ...
  void setup() override {
    ...
    auto buffer_count = round_buffer(WIDTH) * round_buffer(HEIGHT) / FRACTION;
    this->buffer_ = this->allocate_buffer_(buffer_count);
    ...
    if (this->double_buffer_) {
      this->buffer2_ = this->allocate_buffer_(buffer_count);
  ...
  void update() override {
    ...
    for (this->start_line_ = 0; this->start_line_ < this->get_height_internal(); this->start_line_ = this->end_line_) {
      ...
      this->write_to_display_(this->x_low_, this->y_low_, w, h, this->active_buffer_, this->x_low_,
                              this->y_low_ - this->start_line_, round_buffer(this->get_width_internal()) - w - this->x_low_);
      if (this->double_buffer_) {
  ...
  void draw_pixel_at(int x, int y, Color color) override {
    ...
    if (x < 0 || x >= this->get_width_internal() || y < this->start_line_ || y >= this->end_line_)
      return;
    this->active_buffer_[(y - this->start_line_) * round_buffer(this->get_width_internal()) + x] = convert_color(color);
    if (x < this->x_low_) {
      this->x_low_ = x;
    }
    if (x > this->x_high_) {
  ...
  void fill(Color color) override {
    ...
    std::fill_n(this->active_buffer_, (this->end_line_ - this->start_line_) * round_buffer(this->get_width_internal()),
                convert_color(color));
  }
