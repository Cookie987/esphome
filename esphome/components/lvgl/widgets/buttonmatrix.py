...
class ButtonmatrixButtonType(WidgetType):
    ...
    def __init__(self):
        super().__init__("buttonmatrix_btn", LvButtonMatrixButton, (), {}, {})
...
class MatrixButton(Widget):
    ...
    def map_ctrls(state):
        ...
        return getattr(LV_BUTTONMATRIX_CTRL, state)

    def has_state(self, state):
        state = self.map_ctrls(state)
        return lv_expr.BUTTONMATRIX_has_btn_ctrl(self.obj, self.index, state)

    def add_state(self, state):
        state = self.map_ctrls(state)
        return lv.BUTTONMATRIX_set_btn_ctrl(self.obj, self.index, state)

    def clear_state(self, state):
        state = self.map_ctrls(state)
        return lv.BUTTONMATRIX_clear_btn_ctrl(self.obj, self.index, state)
...
async def get_button_data(config, buttonmatrix: Widget):
    ...
    for row in config:
        for button_conf in row.get(CONF_BUTTONS, ()):
            ...
            ctrl = ["LV_BUTTONMATRIX_CTRL_CLICK_TRIG"]
...
def set_btn_data(obj, ctrl_list, width_list):
    for index, ctrl in enumerate(ctrl_list):
        lv.BUTTONMATRIX_set_btn_ctrl(obj, index, ctrl)
    for index, width in enumerate(width_list):
        lv.BUTTONMATRIX_set_btn_width(obj, index, width)
...
async def button_update_to_code(config, action_id, template_arg, args):
    ...
    async def do_button_update(w):
        if (width := config.get(CONF_WIDTH)) is not None:
            lv.BUTTONMATRIX_set_btn_width(w.obj, w.index, width)
        if config.get(CONF_SELECTED):
            lv.BUTTONMATRIX_set_selected_btn(w.obj, w.index)
        if controls := config.get(CONF_CONTROL):
            adds = []
            clrs = []
            for item in controls:
                adds.extend(
                    [f"LV_BUTTONMATRIX_CTRL_{k.upper()}" for k, v in item.items() if v]
                )
                clrs.extend(
                    [f"LV_BUTTONMATRIX_CTRL_{k.upper()}" for k, v in item.items() if not v]
                )
            if adds:
                lv.BUTTONMATRIX_set_btn_ctrl(
                    w.obj, w.index, await BUTTONMATRIX_CTRLS.process(adds)
                )
            if clrs:
                lv.BUTTONMATRIX_clear_btn_ctrl(
