...
void LvglComponent::dump_config() {
  ESP_LOGCONFIG(TAG,
                ...
                YESNO(this->use_dma_), YESNO(this->use_psram_), YESNO(this->double_buffer_));
...
void LvglComponent::flush_cb_(lv_display_t *disp_drv, const lv_area_t *area, uint8_t *color_p) {
  if (!this->is_paused()) {
    auto now = millis();
    if (!this->async_flush_pending_ && this->double_buffer_ && this->rotation_ == display::DISPLAY_ROTATION_0_DEGREES &&
...
LvglComponent::LvglComponent(std::vector<display::Display *> displays, float buffer_frac, bool full_refresh,
                             ...
                             RotationType rotation_type, bool use_dma, bool use_psram, bool double_buffer)
...
void *LvglComponent::allocate_draw_buffer_(size_t size) {
  ...
  if (this->use_psram_) {
    ...
    if (this->use_dma_)
      caps |= MALLOC_CAP_DMA;
    void *ptr = heap_caps_aligned_alloc(LV_DRAW_BUF_ALIGN, size, caps);  // NOLINT
    if (ptr != nullptr) {
      ESP_LOGV(TAG, "Allocated %s draw buffer %zu bytes at %p", this->use_dma_ ? "PSRAM DMA" : "PSRAM", size, ptr);
      return ptr;
    }
    ESP_LOGW(TAG, "Failed to allocate %zu bytes for %s draw buffer", size, this->use_dma_ ? "PSRAM DMA" : "PSRAM");
    return nullptr;
  }
  if (this->use_dma_) {
    void *ptr = heap_caps_aligned_alloc(LV_DRAW_BUF_ALIGN, size,
                                        MALLOC_CAP_INTERNAL | MALLOC_CAP_DMA | MALLOC_CAP_8BIT);  // NOLINT
    if (ptr != nullptr) {
...
#endif
  bool internal = this->buffer_frac_ >= MIN_BUFFER_FRAC / 2;
  void *buffer = lv_alloc_draw_buf(size, internal);  // NOLINT
  if (buffer == nullptr && internal)
    buffer = lv_alloc_draw_buf(size, false);  // NOLINT
  return buffer;
}
...
void LvglComponent::setup() {
  ...
  this->rotation_ = display->get_rotation();
  this->set_resolution_();
  ...
  if (this->rotation_type_ == RotationType::ROTATION_SOFTWARE) {
    this->rotate_buf_ = this->allocate_draw_buffer_(buf_bytes);
